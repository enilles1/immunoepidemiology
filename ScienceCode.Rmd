...
# File path
file_path <- "/Users/ericnilles/Dropbox (Personal)/Dropbox/Cohort Data/Science_df.csv"

# Reading the CSV file into the dataframe dfx
dfx <- read_csv(file_path)
...
# Labels
scale_labels <- c("10^-1", "10^0", "10^1", "10^2", "10^3", "10^4", "10^5")
...
# Create list of subsets and corresponding titles
data_list <- list(dfx, 
                  dfx %>% filter(vacc_dose_interval == 0), 
                  dfx %>% filter(vacc_dose_interval == 1), 
                  dfx %>% filter(vacc_dose_interval == 2), 
                  dfx %>% filter(vacc_dose_interval == 3))
...
plot <- ggplot(data_melt, aes(x = condition, y = value)) +
    ...
    scale_color_manual(values = c( "Negative" = "darkred", "Positive" = "darkgreen")) +
     ...
     scale_y_continuous(name = ifelse(is.null(y_name), "", y_name),
     ...
     theme(panel.grid.major.x = element_blank(),
           ...
           panel.grid.major.y = element_blank(),
           legend.position = "none",
           axis.ticks.y = if (show_y_ticks) { element_line(size = 0.5) } else { element_blank() },
           panel.spacing = unit(1.5, "lines"),
           axis.text.x = element_text(size = 9),
           axis.text.y = if (show_y_ticks) { element_text(size = 9) } else { element_blank() },
           axis.title.y = if (show_y_ticks) { element_text(size = 9) } else { element_blank() },
...
y_names <- c('Log S titer', NULL, NULL, NULL, NULL)

# Define titles
titles <- c("Overall", "Interval vacc = 0", "Interval vacc = 1", "Interval vacc = 2", "Interval vacc = 3")

# Create plots for all data frames
plots <- list()

for (i in seq_along(data_list)) {
  plots[[i]] <- create_ggpaired_plot(data_list[[i]], titles[i], y_names[i], show_y_ticks = (i == 1))
}

# Create the plot for the overall data
fig2a <- create_ggpaired_plot(dfx, NULL, 'Log S titer', TRUE)
...
ggsave("PCA.png", pca_biplot, width = 3, height = 3, units = "in")
...
# Simulate an initial distribution of titres
set.seed(199)
...
hist(initial_distribution,breaks=seq(0,1.1*max(initial_distribution),0.5),xlab="log10 titre",ylab="frequency",
     xaxs="i",yaxs="i",bty="l",lwd=2,col="light green",main="Initial titres",xlim=c(0,8))

# Plot pre-to-post titre function
plot(pre_titre,pre_post_titres(pre_titre),xlim=c(0,6),ylim=c(0,6),xaxs="i",yaxs="i",
...
hist(wave_1_post,breaks=seq(0,1.4*max(wave_1_post),0.5),xlab="log10 titre",ylab="frequency",
     xaxs="i",yaxs="i",bty="l",lwd=2,col="light green",main="Titres after 1 wave",xlim=c(0,8))
...
hist(wave_2_post,breaks=seq(0,1.4*max(wave_2_post),0.5),xlab="log10 titre",ylab="frequency",
     xaxs="i",yaxs="i",bty="l",lwd=2,col="light green",main="Titres after 2 waves",xlim=c(0,8))

mtext("C. Titer-dependent boosting + waning high titers. Simulated initial titers", side = 3, adj = 0, outer = TRUE, cex = 1.2, line = 1)

dev.copy(png,paste0("titre_plot_1.png"),units="cm",width=30,height=8,res=150)
dev.off()

# Plot results for non converging set points ------------------------------------------------------------
...
# Plot distribution of initial titres
hist(initial_distribution,breaks=seq(0,1.1*max(initial_distribution),0.5),xlab="log10 titre",ylab="frequency",
     xaxs="i",yaxs="i",bty="l",lwd=2,col="light gray",main="Initial titres",xlim=c(0,8))

# Plot pre-to-post titre function
plot(pre_titre,pre_post_titres(pre_titre,param1,param2,param3),xlim=c(0,6),ylim=c(0,6),xaxs="i",yaxs="i",
     bty="l",col="dark blue",type="l",lwd=2,xlab="pre-infection titre",ylab="post titre",
     main="Pre-to-post function")
lines(pre_titre,pre_titre,lty=2)
...
hist(wave_1_post,breaks=seq(0,1.4*max(wave_1_post),0.5),xlab="log10 titre",ylab="frequency",
     xaxs="i",yaxs="i",bty="l",lwd=2,col="light gray",main="Titres after 1 wave",xlim=c(0,8))

# Plot titres after 2 waves of infection
wave_2_post <- pre_post_titres(wave_1_post,param1,param2,param3)

hist(wave_2_post,breaks=seq(0,1.4*max(wave_2_post),0.5),xlab="log10 titre",ylab="frequency",
     xaxs="i",yaxs="i",bty="l",lwd=2,col="light gray",main="Titres after 2 waves",xlim=c(0,8))

mtext("B. Titer-dependent boosting + no waning. Simulated initial titers", side = 3, adj = 0, outer = TRUE, cex = 1.2, line = 1)

dev.copy(png,paste0("titre_plot_2.png"),units="cm",width=30,height=8,res=150)
dev.off()
...
# Plot distribution of initial titres
hist(initial_distribution,breaks=seq(0,1.1*max(initial_distribution),0.5),xlab="log10 titre",ylab="frequency",
     xaxs="i",yaxs="i",bty="l",lwd=2,col="light blue",main="Initial titres",xlim=c(0,8))

# Plot pre-to-post titre function
plot(pre_titre,pre_post_titres(pre_titre,param1,param2,param3,param4),xlim=c(0,6),ylim=c(0,6),xaxs="i",yaxs="i",
     bty="l",col="dark blue",type="l",lwd=2,xlab="pre-infection titre",ylab="post titre",
     main="Pre-to-post function")
lines(pre_titre,pre_titre,lty=2)
...
mtext("A. Titer-independent boosting + no waning. Simulated initial titers", side = 3, adj = 0, outer = TRUE, cex = 1.2, line = 1)

dev.copy(png,paste0("titre_plot_3.png"),units="cm",width=30,height=8,res=150)
dev.off()
...
# Import data
initial_distribution <- dfx$s_titer_orig_log |> pmax(0)
...
mtext("F. Titer-dependent boosting + waning high titers. Observed initial titers",  side = 3, adj = 0, outer = TRUE, cex = 1.2, line = 1)

dev.copy(png,paste0("titre_plot_1_data.png"),units="cm",width=30,height=8,res=150)
dev.off()
...
mtext("E. Titer-dependent boosting + no waning. Observed initial titers",  side = 3, adj = 0, outer = TRUE, cex = 1.2, line = 1)

dev.copy(png,paste0("titre_plot_2_data.png"),units="cm",width=30,height=8,res=150)
dev.off()
...
mtext("D. Titer-independent boosting + no waning - Observed initial titers", side = 3, adj = 0, outer = TRUE, cex = 1.2, line = 1)

dev.copy(png,paste0("titre_plot_3_data.png"),units="cm",width=30,height=8,res=150)
dev.off()
